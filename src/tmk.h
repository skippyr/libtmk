...
/**
 * Contains the first 16 ANSI colors of the XTerm color palette. They map to the colors of the current terminal theme. A color may be set by using the tmk_setFontAnsiColor
 * function.
 */
enum tmk_AnsiColor
{
...
/** Contains the terminal layers. They are used as reference to set colors when using the tmk_setFontAnsiColor and tmk_setFontRgbColor functions. */
enum tmk_Layer
{
...
/** Contains the terminal font weights. A weight may be set by using the tmk_setFontWeight function. */
enum tmk_FontWeight
{
...
/** Contains the available terminal font effects. One or multiple effects may be set by using the tmk_setFontEffects function. */
enum tmk_FontEffect
{
...
/** Contains the terminal cursor shapes. A shape may be set by using the tmk_setCursorShape function. */
...
/** Contains the keyboard keys that are not represented by UTF-8 graphemes. They may be set in a tmk_KeyEvent struct by the tmk_readKeyEvent function. */
enum tmk_Key
{
#if tmk_IS_OPERATING_SYSTEM_WINDOWS
    /** The Left Arrow key. */
...
/**
 * Contains the keyboard modifier keys. The Shift key may be identified by checking the key read during events. They may be set in a tmk_KeyEvent struct by the tmk_readKeyEvent
 * function.
 */
...
/** Contains the standard terminal streams. Their statuses may be check by using the tmk_isStreamRedirected function. */
enum tmk_Stream
{
...
/** Represents the command line arguments. It may be filled by using the tmk_getCmdArguments function. */
...
/**
 * Represents a terminal coordinate. It uses a coordinate system that considers the origin position at the top left corner of the screen, where both column and row components are
 * zero. From there, those components increase going right and down, respectively. It is used by the tmk_getCursorCoordinate and tmk_setCursorCoordinate functions.
 */
...
/** Represents terminal dimensions. It is used by the tmk_getWindowDimensions function. */
...
/** Represents an RGB color. It may be set by using the tmk_setFontRgbColor function. */
struct tmk_RgbColor
{
...
/** Represents a terminal key event. It may be filled by the tmk_readKeyEvent function. */
struct tmk_KeyEvent
{
    /** A bitmask containing the modifier keys being hold during the event. It may be composed by enumerators from the tmk_ModifierKey enum. */
    unsigned char modifiers;
    /** The key pressed. It may be an UTF-8 grapheme or an enumerator from the tmk_Key enum. */
    int key;
};
...
extern "C"
{
     ...
     * @param length The address where the length of the converted string without including the NULL grapheme at its end will be put into. It is equivalent to the total bytes in
     * the string. It may be freed by using the free function. If not necessary, set it to NULL.
     ...
     * @param length The address where the length of the converted string without including the NULL grapheme at its end will be put into. It is equivalent to the total of
     * graphemes, being 2 bytes each, in the string, as that is the measurement unit use by the Windows API UTF-16 functions, even though that encoding supports graphemes up to 4
     * bytes. It may be freed by using the free function. If not necessary, set it to NULL.
    ...
    extern void tmk_ringBell(void);
    /** Clears the line the terminal is on. */
    extern void tmk_clearCursorLine(void);
    /**
     * Gets and converts the command line arguments of the terminal process. It fills a tmk_CommandLineArguments struct which must be freed by using the tmk_freeCmdArguments
     * function.
     * @param totalMainArguments The total command line arguments given as the first parameter of the main function.
     * @param mainArguments The raw command line arguments given as the second parameter of the main function.
     * @param arguments The address where the command line arguments will be put into.
     */
    ...
    /**
     * Sets an ANSI color into the terminal font. The color may be reset by using the tmk_resetFontColors function.
     * @param color The color to be applied. It may be a value in range from 0 to 255 or an enumerator from the tmk_AnsiColor enum.
     * @param layer The layer to be affected.
     */
    extern void tmk_setFontAnsiColor(unsigned char color, enum tmk_Layer layer);
    /**
     * Sets an RGB color into the terminal font. The color may be reset by using the tmk_resetFontColors function.
    ...
    /**
     * Sets the terminal font weight. The weight may be reset by using the tmk_resetFontWeight function.
    ...
    /**
     * Sets terminal font effects. Effects may be reset by using the tmk_resetFontEffects function.
     * @param effectsMask A bitmask containing the effects to applied. It must be composed by enumerators from the tmk_FontEffect enum.
     */
    extern void tmk_setFontEffects(int effectsMask);
    /** Resets all terminal font effects. */
    extern void tmk_resetFontEffects(void);
    /** Opens the alternate window. It may be closed by using the tmk_closeAlternateWindow function. On Windows, it causes a window resize event. */
     ...
     * @param coordinate The coordinate to be set. It will always be fit within the terminal window boundaries.
     */
    extern void tmk_setCursorCoordinate(struct tmk_Coordinate coordinate);
    /**
     * Sets the terminal cursor shape. It may be reset by using the tmk_resetCursorShape function.
    ...
    /**
     * Reads a terminal key event and remove it from the input buffer. Content typed and pasted into the terminal are considered key events. Under some situations, the terminal may
     * not give enough information for the library to distinguish certain key events. Some key sequences may be reserved by the terminal or operating system. Modifiers keys may
     * only be identified in keys represented by UTF-8 graphemes. During execution, it blocks the receipt of signals. For convenience, it flushes the terminal output buffer upon
     * execution. To avoid reading cached events, use it with the tmk_clearInputBuffer function.
     * @param waitInMilliseconds The time in milliseconds to wait for an event. If the time is negative, it waits forever; If zero, it returns immediately; Or, if positive, it
     * waits the given time.
     * @param event The address where the event information will be put into. If not necessary, it may be set to NULL.
     * @param filter a function to filter events while the timer is running, avoiding to destroy it unless necessary. It returns a boolean that states the event should be
     * considered. It is only used when the wait specified is positive. If not necessary, set it to NULL.
     * @return 0 if successful; -1 if user interaction was not possible due to stream redirection or wide character orientation; -2 if no events were available; -3 if the timer
     * ran out; Or -4 if the terminal window was resized during the reading.
     */
    ...
    /**
     * Formats and writes a string to the terminal error stream. On Windows, it sets the console output code page to UTF-8. In order to synchronize possible ANSI escape sequences
    ...
    /**
     * Formats and writes a string to the terminal error stream with a newline grapheme appended to its end. On Windows, it sets the console output code page to UTF-8. In order to
     * synchronize possible ANSI escape sequences cached, it flushes the terminal output buffer upon execution.
     * @param format The format to be used. It accepts the same format specifiers as the printf function family.
     * @param arguments The arguments to be formatted.
     */
