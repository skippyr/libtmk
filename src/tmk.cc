...
namespace tmk {
    RgbColor::RgbColor(uint8_t red, uint8_t green, uint8_t blue) noexcept : red_(red), green_(green), blue_(blue)
    {
    }
    ...
    Coordinate::Coordinate(uint16_t column, uint16_t row) noexcept : column_(column), row_(row)
    {
    }
    ...
    std::string Encoding::convertUtf16ToUtf8(const std::wstring& utf16String)
    {
        ...
        WideCharToMultiByte(CP_UTF8, 0, utf16String.c_str(), -1, buffer.get(), size, nullptr, nullptr);
        return buffer.get();
    }
    ...
    MultiEncodingString::MultiEncodingString(const std::string& utf8String)
        : utf8String_(utf8String), utf16String_(Encoding::convertUtf8ToUtf16(utf8String))
    {
    }

    MultiEncodingString::MultiEncodingString(const std::wstring& utf16String)
        : utf8String_(Encoding::convertUtf16ToUtf8(utf16String)), utf16String_(utf16String)
    {
    }
#else
    MultiEncodingString::MultiEncodingString(const std::string& utf8String) : utf8String_(utf8String)
    {
    }
#endif
    ...
    void Terminal::setRawInput(bool isRaw) noexcept
    {
        ...
        attributes.c_lflag =
            isRaw ? attributes.c_lflag & ~(ICANON | ECHO | ISIG) : attributes.c_lflag | ICANON | ECHO | ISIG;
        attributes.c_iflag = isRaw ? attributes.c_iflag & ~IXON : attributes.c_iflag | IXON;
    ...
    Dimensions Terminal::getWindowDimensions()
    {
        ...
        if (ioctl(STDIN_FILENO, TIOCGWINSZ, &ioctlSize) && ioctl(STDOUT_FILENO, TIOCGWINSZ, &ioctlSize) &&
