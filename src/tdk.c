...
#ifdef _WIN32
#define _tdk_TTY_CACHE(a_file, a_stream)                                       \
  (!!_isatty(_fileno(a_file)) << a_stream)
#else
...
int tdk_getCursorCoordinate(struct tdk_Coordinate *coordinate) {
  ...
  int totalMatchesRead =
      scanf("\033[%hu;%huR", &coordinate->row, &coordinate->column);
  attributes.c_lflag |= ICANON | ECHO;
  tcsetattr(STDIN_FILENO, TCSANOW, &attributes);
...
void tdk_setRGBColor(struct tdk_RGB color, int layer) {
  _tdk_writeANSI("\033[%d8;2;%d;%d;%dm", layer, color.red, color.green,
                 color.blue);
}
