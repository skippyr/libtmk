...
static void _tdk_cacheTTY(void)
{
    if (!(_tdk_cache_g & 1 << 7))
    {
        _tdk_cache_g |= isatty(STDIN_FILENO) | isatty(STDOUT_FILENO) << 1 | isatty(STDERR_FILENO) << 2 | 1 << 7;
    }
}
...
int tdk_readKeyEvent(struct tdk_KeyEvent *event)
{
    ...
    while (1)
    {
        ...
        if (buffer[0] & 1 << 7)
        {
            for (int offset = 1; offset < 1 + !!(buffer[0] & 1 << 6) + !!(buffer[0] & 1 << 5) + !!(buffer[0] & 1 << 4);
                 ++offset)
            {
                buffer[offset] = getchar();
            }
        ...
        else if ((event->key = (event->modifiers = buffer[0] == 27 && buffer[1] != EOF) ? buffer[1] : buffer[0]) >= 0 &&
