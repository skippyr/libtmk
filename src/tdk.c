...
#define _tdk_HAS_TTY_CACHE (_tdk_cache_g & _tdk_HAS_TTY_CACHE_FLAG)
...
#ifdef _WIN32
#define _tdk_TTY_CACHE(file_a, stream_a) \
  (!!_isatty(_fileno(file_a)) << stream_a)
#else
#define _tdk_TTY_CACHE(file_a, stream_a) (!!isatty(fileno(file_a)) << stream_a)
#endif
...
#endif
  _tdk_cache_g |= _tdk_TTY_CACHE(stdin, tdk_Stream_Input) |
                  _tdk_TTY_CACHE(stdout, tdk_Stream_Output) |
                  _tdk_TTY_CACHE(stderr, tdk_Stream_Error) |
                  _tdk_HAS_TTY_CACHE_FLAG;
}
...
static int _tdk_writeANSI(const char *format, ...) {
  ...
  int totalBytesWritten = vfprintf(
      _tdk_IS_TTY(tdk_Stream_Output) ? stdout : stderr, format, arguments);
  va_end(arguments);
...
int tdk_getWindowDimensions(struct tdk_Dimensions *dimensions) {
  ...
  dimensions->totalColumns = windowSize.ws_col;
  dimensions->totalColumns = windowSize.ws_row;
#endif
  return 0;
}
    ...
    *buffer = getchar();
    ...
    if (*buffer & 1 << 7) {
      ...
      event->modifiers = 0;
                    ...
                    (event->modifiers = *buffer == 27 && buffer[1] != EOF)
                        ? buffer[1]
                        : *buffer) >= 0 &&
...
void tdk_set256Color(int color, int layer) {
  _tdk_writeANSI(color == tdk_Color_Default ? "\x1b[%d9m" : "\x1b[%d8;5;%dm",
                 layer, color);
}
...
int tdk_setCursorCoordinate(struct tdk_Coordinate coordinate) {
  ...
  return -(
      tdk_getWindowDimensions(&windowDimensions) || ++coordinate.column <= 0 ||
...
void tdk_setHEXColor(int color, int layer) {
  tdk_setRGBColor(
      (struct tdk_RGB){color >> 16 & 0xff, color >> 8 & 0xff, color & 0xff},
      layer);
}

void tdk_setRGBColor(struct tdk_RGB color, int layer) {
  _tdk_writeANSI("\x1b[%d8;2;%d;%d;%dm", layer, color.red, color.green,
                 color.blue);
}

void tdk_setWeight(int weight) {
  _tdk_writeANSI(weight == tdk_Weight_Default ? "\x1b[22m" : "\x1b[22;%dm",
                 weight);
}
